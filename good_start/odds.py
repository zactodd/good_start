...
def good_game_selection_with_tray_and_humingbird(sample=1000):
    good_games = Counter()
    for cards in map(lambda _: random.sample(BIRD_NAMES, 8), range(sample)):
        hand = cards[:5]
        tray = cards[5:]
        for bird in BIRD_IMPORTANCE:
            required_birds = bird['birds']
            required_tray = bird['tray']
            if all(b in hand for b in required_birds) and \
                    any(b in BIRD_GROUPS and 'Hummingbird' in BIRD_GROUPS[b] for b in tray) and \
                any(b in required_tray and not (b in BIRD_GROUPS and 'Hummingbird' in BIRD_GROUPS[b]) for b in tray):
                name = '_'.join(''.join(w[0] for w in BIRD_GROUPS[b].split()) for b in required_birds)
                good_games[name] += 1
    good_games = {k: v / sample for k, v in good_games.items()}
    plt.bar(good_games.keys(), good_games.values())
...
good_game_selection_with_tray_and_humingbird(n)
